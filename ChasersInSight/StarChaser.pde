...
class StarChaser {
  ...
  void update() {
    if ( animate ) {
      ...
      float head_ex = constrain(1 - (1 - animationState)*(1 - animationState), 0, 1);
      float tail_ex = constrain(1 - (1 - animationState)*(1 - animationState)*(1 - animationState), 0, 1);
      head = PVector.lerp(origin, destination, head_ex);
      tail = PVector.lerp(origin, destination, tail_ex);
  ...
  void refreshSearchingArea() {
    searchArea = new SearchingArea(PVector.lerp(origin, chaserTarget, 0.9), 120);
  }
